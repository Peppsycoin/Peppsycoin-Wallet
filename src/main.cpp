...
const string strMessageMagic = "peppsycoin Signed Message:\n";
...
{
    int64 nSubsidy = 95 * COIN;

    // Subsidy is cut in half every 788400 blocks, which will occur approximately every 3/4 years
    nSubsidy >>= (nHeight / 788400); // Peppsycoin: 788400 blocks in 3/4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 0.017361111 * 24 * 60 * 60; // peppsycoin:1,4min
static const int64 nTargetSpacing = 30; // peppsycoin: 30sec blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlock *pblock)
{
    ...
    // peppsycoin: This fixes an issue where a 51% attack can change difficulty at will.
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal peppsycoin miner
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "peppsycoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // peppsycoind: Reduce the exempted free transactions to 500 bytes (from Bitcoin's 3000 bytes)
